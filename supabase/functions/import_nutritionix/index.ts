...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!nutritionixAppId || !nutritionixApiKey) {
      return new Response(
        JSON.stringify({ error: 'Nutritionix credentials not configured' }),
    ...
    if (!chainKey) {
      return new Response(
        JSON.stringify({ error: 'chainKey is required' }),
    ...
    if (!brandIds || brandIds.length === 0) {
      return new Response(
        JSON.stringify({ error: `No Nutritionix mapping found for chain: ${chainKey}` }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (brandError || !brand) {
      return new Response(
        JSON.stringify({ error: `Brand not found: ${chainKey}` }),
    ...
    for (const nutritionixBrandId of brandIds) {
      ...
      try {
        ...
        const brandItems = items.filter((item: any) => 
          item.brand_name_item_name?.toLowerCase().includes(chainKey) ||
          item.nix_brand_id === nutritionixBrandId
        );
